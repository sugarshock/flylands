...
namespace Flylands.MarchingCubesProject
{
    public abstract class Marching
    {
        ...
        public virtual void Generate(VoxelArray voxels, IList<Vector3> verts, IList<int> indices, IList<Vector3> uv1, IList<Vector3> uv2)
        {

            int width = voxels.Voxels.GetLength(0);
            int height = voxels.Voxels.GetLength(1);
            int depth = voxels.Voxels.GetLength(2);
            ...
            for (x = 0; x < width - 1; x++)
            {
                for (y = 0; y < height - 1; y++)
                {
                    for (z = 0; z < depth - 1; z++)
                    {   
                        var cubeTerrains = new TerrainType[8];
                        ...
                        for (i = 0; i < 8; i++)
                        {
                            ...
                            cubeTerrains[i] = voxels.Terrains[ix, iy, iz];

                        }
                        //Perform algorithm
                        March(x, y, z, Cube, cubeTerrains, verts, indices, uv1, uv2);
                    }
                }
            }
        ...
        /// </summary>
        protected abstract void March(float x, float y, float z, float[] cube, TerrainType[] cubeTerrains, IList<Vector3> vertList, IList<int> indexList, IList<Vector3> uv1, IList<Vector3> uv2);
