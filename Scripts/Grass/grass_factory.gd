...
static func generate(
	...
	p_sway_yaw:Vector2) -> Array:
		...
		var UV2s:PackedVector2Array = surface[Mesh.ARRAY_TEX_UV2]
		for index in range(0, indices.size(), 3):
			var j = indices[index]
			var k = indices[index + 1]
			var l = indices[index + 2]
			
			var avrg_normal = from_bcc_vector3(
					Vector3(1,1,1),
					normals[j],
					normals[k],
					normals[l]
			)
			if avrg_normal.y >= 0.95:
				...
				for _i in range(0, blades_per_face):
					...
					var transform:Transform3D = Transform3D(Basis(q1), position)
...
func top_color_from(bottom_color:Color):
	return bottom_color.lightened(0.2)
